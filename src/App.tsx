  ...
  return (
    <div style={{ width: '100vw', height: '100vh', backgroundColor: '#000', position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{ position: 'absolute', bottom: '30px', right: '40px', zIndex: 10, display: 'flex', gap: '10px' }}>
        <button onClick={toggleFullScreen} style={{ padding: '12px 15px', backgroundColor: 'rgba(0,0,0,0.5)', border: '1px solid #FFD700', color: '#FFD700', fontFamily: 'sans-serif', fontSize: '18px', fontWeight: 'bold', cursor: 'pointer', backdropFilter: 'blur(4px)' }} title="Toggle Fullscreen">
          ⛶
        </button>
...
const BackgroundMusic = () => {
  ...
  useEffect(() => {
    ...
    // Global click listener to start music if autoplay failed
  ...
  const toggleMusic = (e: React.MouseEvent) => {
    e.stopPropagation(); // Prevent triggering the global listener logic redundantly
    if (audioRef.current) {
      if (isPlaying) {
  ...
  return (
    <div style={{ position: 'absolute', top: '30px', right: '40px', zIndex: 11, display: 'flex', alignItems: 'center', gap: '8px' }}>
      <audio ref={audioRef} loop src="/music.mp3" />
      {!isPlaying && !hasInteracted && (
        <span style={{ color: 'rgba(255,255,255,0.7)', fontSize: '10px', textTransform: 'uppercase', pointerEvents: 'none' }}>
      ...
      <button
        ...
        style={{
          ...
          border: 'none',
          cursor: 'pointer',
          color: isPlaying ? '#FFD700' : 'rgba(255, 255, 255, 0.5)',
          fontSize: '24px',
          transition: 'transform 0.2s cubic-bezier(0.175, 0.885, 0.32, 1.275), color 0.3s ease',
          transform: isPlaying ? 'scale(1.1)' : 'scale(1)',
          textShadow: isPlaying ? '0 0 10px #FFD700' : 'none'
        }}
